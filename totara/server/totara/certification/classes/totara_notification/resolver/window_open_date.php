...
class window_open_date extends notifiable_event_resolver implements scheduled_event_resolver, permission_resolver {
    /**
     * @inheritDoc
     */
    public static function get_notification_title(): string {
        return get_string('notification_window_open_date_resolver_title', 'totara_certification');
    }
    ...
    public static function get_scheduled_events(int $min_time, int $max_time): moodle_recordset {
        ...
        if (!defined('CERTIFSTATUS_COMPLETED')) {
            require_once("{$CFG->dirroot}/totara/certification/lib.php");
        }

        $sql = "SELECT prog.id AS program_id, cc.userid AS user_id, cc.timewindowopens AS time_window_opens
                  FROM {certif_completion} cc
                  JOIN {prog} prog ON prog.certifid = cc.certifid
                  JOIN {user} u ON u.id = cc.userid
                 WHERE cc.timewindowopens >= :min_time
                   AND cc.timewindowopens < :max_time
                   AND u.deleted = 0
                   AND u.suspended = 0";
