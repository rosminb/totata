...
class reportbuilder {
    ...
    public $fullname, $shortname, $source, $hidden, $searchcolumns, $requiredcolumns, $requiredjoins, $initialdisplay;
    ...
    */
    ...
    function get_required_joins() {
        ...
        foreach ($this->requiredjoins as $join) {
            if (in_array($join->dependencies, $joinlistnames)) {
    ...
    protected function collect_joins($filtered, $countonly = false) {
        ...
        $joins = array_merge($requiredjoins, $columnjoins, $filterjoins, $paramjoins, $contentjoins, $sourcejoins, $globalrestrictionjoins);
     ...
     */
    function export_data($format, ?bool $validate_sesskey = true) {
        global $PAGE;
    ...
    public function post_config_visibility_where($type, $table, $userid = null, $showhidden = false, $fieldid = 'id',
        ...
        // Report should not be cached as we are dealing here with visibility.
        if ($this->is_cached()) {
            debugging('Report is cached. This report does visibility checks which needs to be calculated using live data.');
        }
