...
function profile_signup_manager($mform, $nojs, $managerjaid = 0) {
    ...
    if ($managerjaid) {
        ...
        $managername = totara_job_display_user_job($manager, $managerja);
    }
...
function profile_has_required_custom_fields_set($userid) {
    ...
    $sql = "SELECT f.id
         ...
         LEFT JOIN {user_info_data} d ON (d.fieldid = f.id AND d.userid = ?)
             WHERE f.required = 1 AND f.visible > 0 AND f.locked = 0 
               AND (d.id IS NULL OR d.data = '' OR (f.datatype = 'checkbox' AND d.data = '0'))";
