...
class core_setuplib_testcase extends advanced_testcase {
    ...
    public function test_get_docs_url_standard() {
        ...
        $this->assertMatchesRegularExpression('~^' . preg_quote($docroot, '') . '/display/(TH\d{2}|latest)/course/editing$~',
    ...
    public function test_localcachedir() {
        ...
        $this->set_filesystem_safe_current_time_start();
        ...
        $now = $this->set_filesystem_safe_current_time_start();
    ...
    /**
     * Now to reliably compare file modification times we also need
     * the start time to be based upon a file modification time
     *
     * @return void
     */
    private function set_filesystem_safe_current_time_start(): int {
        global $CFG;
        $temp_file = $CFG->tempdir.'/.starttime';
        touch($temp_file);
        $time_start = $this->setCurrentTimeStart(filemtime($temp_file));
        unlink($temp_file);
