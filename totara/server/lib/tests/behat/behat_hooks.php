...
class behat_hooks extends behat_base {
     ...
     * @var bool
     */
    private $look_for_js_errors = false;
    ...
    const IGNORE_TAG = 'ignore-js-logging';
    ...
    public function before_scenario(BeforeScenarioScope $scope) {
        ...
        if ($this->running_javascript()) {
            $this->setup_js_error_handling($scope);
     ...
     */
    public function setup_js_error_handling(BeforeScenarioScope $scope) {
        if (!$this->running_javascript()) {
            return;
        }
        ...
        $this->look_for_js_errors = true;
        $this->scenario_data = basename($scope->getFeature()->getFile()) . '.' . $scope->getScenario()->getLine();
    }
     ...
     * @throws \Exception
     */
    public function look_for_js_errors(AfterStepScope $scope) {
        if (!$this->look_for_js_errors) {
            return;
        }
        ...
        try {
            ...
            // Well if the window cannot be retrieved don't bother
            return;
        }
        ...
        // We have some errors which we can safely ignore. State them here:
        $errors_to_ignore = [
            'ResizeObserver loop limit exceeded'
        ];

        $errors = array_filter($errors, function (string $error) use ($errors_to_ignore) {
            foreach ($errors_to_ignore as $error_to_ignore) {
                if (preg_match("/{$error_to_ignore}/", $error)) {
                    return false;
                }
            }
            return true;
        ...
        $message = sprintf("Found %d javascript error%s", count($errors), count($errors) > 1 ? 's' : '');
        ...
        foreach ($errors as $index => $error) {
            $content .= sprintf("   #%d: %s", $index, $error) . PHP_EOL;
        }
