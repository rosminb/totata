...
function enrol_get_instances($courseid, $enabled) {
    ...
    if (!$enabled) {
        return $DB->get_records_sql(
            "SELECT * FROM {enrol} WHERE courseid = :courseid AND status <> :status ORDER BY sortorder, id",
            array(
                'courseid' => $courseid,
                'status' => ENROL_INSTANCE_DELETED,
            )
        );
    }
...
function get_enrolled_sql(context $context, $withcapability = '', $groupid = 0, $onlyactive = false, $onlysuspended = false) {
    ...
    $sql = "SELECT {$prefix}u.id
              FROM {user} {$prefix}u
            $capjoin->joins
             WHERE $capjoin->wheres
             GROUP BY {$prefix}u.id";
